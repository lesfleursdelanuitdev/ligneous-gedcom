...
func BuildGraphLazy(tree *gedcom.GedcomTree) (*Graph, error) {
	graph := NewGraph(tree)
	graph.lazyMode = true
...
func createNodeSkeleton(graph *Graph, tree *gedcom.GedcomTree) error {
...
func detectComponents(graph *Graph, tree *gedcom.GedcomTree) error {
	graph.mu.Lock()
