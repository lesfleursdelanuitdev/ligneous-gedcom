...
type DuplicateDetector struct {
	...
	tree   *gedcom.GedcomTree // Optional: for relationship matching
}
...
func (dd *DuplicateDetector) FindDuplicates(tree *gedcom.GedcomTree) (*DuplicateResult, error) {
	...
	individuals := make([]*gedcom.IndividualRecord, 0, len(allIndividuals))
...
func (dd *DuplicateDetector) FindDuplicatesBetween(tree1, tree2 *gedcom.GedcomTree) (*DuplicateResult, error) {
	...
	individuals1 := make([]*gedcom.IndividualRecord, 0, len(allIndi1))
	...
	individuals2 := make([]*gedcom.IndividualRecord, 0, len(allIndi2))
...
func (dd *DuplicateDetector) compare(indi1, indi2 *gedcom.IndividualRecord) (*DuplicateMatch, error) {
...
func (dd *DuplicateDetector) analyzeFields(indi1, indi2 *gedcom.IndividualRecord,
...
type indexes struct {
	surnameIndex   map[string][]*gedcom.IndividualRecord // surname -> individuals
	birthYearIndex map[int][]*gedcom.IndividualRecord    // birth year -> individuals
	placeIndex     map[string][]*gedcom.IndividualRecord // place (normalized) -> individuals
}

// buildIndexes builds indexes for pre-filtering.
func (dd *DuplicateDetector) buildIndexes(individuals []*gedcom.IndividualRecord) *indexes {
	idx := &indexes{
		surnameIndex:   make(map[string][]*gedcom.IndividualRecord),
		birthYearIndex: make(map[int][]*gedcom.IndividualRecord),
		placeIndex:     make(map[string][]*gedcom.IndividualRecord),
	}
...
func (dd *DuplicateDetector) shouldCompare(indi1, indi2 *gedcom.IndividualRecord, idx *indexes) bool {
...
func (dd *DuplicateDetector) shouldCompareCrossFile(indi1, indi2 *gedcom.IndividualRecord,
