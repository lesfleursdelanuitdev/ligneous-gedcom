...
func TestRelationshipSimilarity(t *testing.T) {
	...
	fam.FirstLine().AddChild(gedcom.NewGedcomLine(1, "HUSB", "@I10@", ""))
	fam.FirstLine().AddChild(gedcom.NewGedcomLine(1, "WIFE", "@I11@", ""))
	tree.AddRecord(fam)
	...
	indi1.FirstLine().AddChild(gedcom.NewGedcomLine(1, "FAMC", "@F1@", ""))
	tree.AddRecord(indi1)

	indi2 := createTestIndividual("John /Doe/", "John", "Doe", "1800", "New York")
	indi2.FirstLine().XrefID = "@I2@"
	indi2.FirstLine().AddChild(gedcom.NewGedcomLine(1, "FAMC", "@F1@", ""))
...
func TestRelationshipSimilarity_CommonSpouse(t *testing.T) {
	...
	famLine := gedcom.NewGedcomLine(0, "FAM", "", "@F1@")
	fam := gedcom.NewFamilyRecord(famLine)
	fam.FirstLine().AddChild(gedcom.NewGedcomLine(1, "HUSB", "@I1@", ""))
	fam.FirstLine().AddChild(gedcom.NewGedcomLine(1, "WIFE", "@I20@", ""))
	tree.AddRecord(fam)

	// Create two individuals with same spouse
	indi1 := createTestIndividual("John /Doe/", "John", "Doe", "1800", "New York")
	indi1.FirstLine().XrefID = "@I1@"
	indi1.FirstLine().AddChild(gedcom.NewGedcomLine(1, "FAMS", "@F1@", ""))
	tree.AddRecord(indi1)

	indi2 := createTestIndividual("John /Doe/", "John", "Doe", "1800", "New York")
	indi2.FirstLine().XrefID = "@I2@"
	indi2.FirstLine().AddChild(gedcom.NewGedcomLine(1, "FAMS", "@F1@", ""))
	tree.AddRecord(indi2)
