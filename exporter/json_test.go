...
func TestJsonExporter_ExportToString(t *testing.T) {
	...
	nameLine := types.NewGedcomLine(1, "NAME", "Test Submitter", "")
	submLine.AddChild(nameLine)
	submRecord := types.NewSubmitterRecord(submLine)
	tree.AddRecord(submRecord)

	// Create individual
	indiLine := types.NewGedcomLine(0, "INDI", "", "@I1@")
	nameLine2 := types.NewGedcomLine(1, "NAME", "John /Doe/", "")
...
func TestJsonExporter_ExportToFile(t *testing.T) {
	...
	submLine := types.NewGedcomLine(0, "SUBM", "", "@U1@")
	nameLine := types.NewGedcomLine(1, "NAME", "Test", "")
	submLine.AddChild(nameLine)
	submRecord := types.NewSubmitterRecord(submLine)
	tree.AddRecord(submRecord)
...
func TestJsonExporter_IndividualToJSON(t *testing.T) {
	...
	givnLine := types.NewGedcomLine(2, "GIVN", "John", "")
	surnLine := types.NewGedcomLine(2, "SURN", "Doe", "")
	nameLine.AddChild(givnLine)
	nameLine.AddChild(surnLine)
	indiLine.AddChild(nameLine)

	birtLine := types.NewGedcomLine(1, "BIRT", "", "")
	dateLine := types.NewGedcomLine(2, "DATE", "1 Jan 1900", "")
	placLine := types.NewGedcomLine(2, "PLAC", "New York", "")
...
func TestJsonExporter_FamilyToJSON(t *testing.T) {
	...
	chilLine := types.NewGedcomLine(1, "CHIL", "@I3@", "")
