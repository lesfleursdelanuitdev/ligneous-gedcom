...
func NewYAMLExporter(errorManager *types.ErrorManager) *YAMLExporter {
	return &YAMLExporter{
...
func (ye *YAMLExporter) ExportToFile(tree *types.GedcomTree, filePath string) error {
	yamlData, err := ye.createYAMLStructure(tree)
	if err != nil {
		ye.AddError(types.SeveritySevere,
...
func (ye *YAMLExporter) ExportToString(tree *types.GedcomTree) (string, error) {
	yamlData, err := ye.createYAMLStructure(tree)
	if err != nil {
		return "", fmt.Errorf("failed to create YAML structure: %w", err)
	}
...
func (ye *YAMLExporter) createYAMLStructure(tree *types.GedcomTree) (*YAMLGedcom, error) {
