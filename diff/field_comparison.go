...
func (gd *GedcomDiffer) compareIndividual(indi1, indi2 *types.IndividualRecord) []FieldChange {
...
func (gd *GedcomDiffer) compareFamily(fam1, fam2 *types.FamilyRecord) []FieldChange {
	changes := make([]FieldChange, 0)
...
func (gd *GedcomDiffer) compareBasicRecord(record1, record2 types.Record) []FieldChange {
	changes := make([]FieldChange, 0)
...
func (gd *GedcomDiffer) areDatesSemanticallyEquivalent(date1, date2 string) bool {
	...
	// Try to parse dates
	parsed1, err1 := types.ParseDate(date1)
	parsed2, err2 := types.ParseDate(date2)
