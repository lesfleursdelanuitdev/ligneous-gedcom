...
func (gd *GedcomDiffer) compareByXref(tree1, tree2 *types.GedcomTree) (DiffChanges, error) {
	changes := DiffChanges{
...
func (gd *GedcomDiffer) compareRecordsByXref(
	records1, records2 map[string]types.Record,
	recordType string,
) DiffChanges {
	changes := DiffChanges{
		Added:    make([]RecordDiff, 0),
...
func (gd *GedcomDiffer) compareRecordContent(
	...
	switch recordType {
	case "INDI":
		indi1, ok1 := record1.(*types.IndividualRecord)
		indi2, ok2 := record2.(*types.IndividualRecord)
		if ok1 && ok2 {
			fieldChanges := gd.compareIndividual(indi1, indi2)
			changes = append(changes, fieldChanges...)
		}
	case "FAM":
		fam1, ok1 := record1.(*types.FamilyRecord)
		fam2, ok2 := record2.(*types.FamilyRecord)
		if ok1 && ok2 {
