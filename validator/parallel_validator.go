...
func NewParallelGedcomValidator(errorManager *types.ErrorManager) *ParallelGedcomValidator {
...
func (pgv *ParallelGedcomValidator) Validate(tree *types.GedcomTree) error {
...
func (pgv *ParallelGedcomValidator) validateSubmitter(tree *types.GedcomTree) {
	header := tree.GetHeader()
	if header != nil {
		submXref := header.GetValue("SUBM")
		if submXref == "" {
			pgv.errorManager.AddError(types.SeveritySevere,
				"Missing SUBM record",
				0,
...
func (pgv *ParallelGedcomValidator) GetErrorManager() *types.ErrorManager {
