...
func NewDateConsistencyValidator(errorManager *types.ErrorManager) *DateConsistencyValidator {
...
func (dcv *DateConsistencyValidator) Validate(tree *types.GedcomTree, config *ValidationConfig) []*types.GedcomError {
	errors := make([]*types.GedcomError, 0)
...
func (dcv *DateConsistencyValidator) validateIndividualDates(indi *types.IndividualRecord, xrefID string, tree *types.GedcomTree, config *ValidationConfig) []*types.GedcomError {
	errors := make([]*types.GedcomError, 0)
	...
	if birthDateStr == "" {
		errors = append(errors, &types.GedcomError{
...
func (dcv *DateConsistencyValidator) validateFamilyDates(fam *types.FamilyRecord, xrefID string, tree *types.GedcomTree, config *ValidationConfig) []*types.GedcomError {
	errors := make([]*types.GedcomError, 0)
	...
	if marriageDate != nil && marriageDate.IsValid() {
		...
		for _, childXref := range children {
			...
			child, ok := childRecord.(*types.IndividualRecord)
			if !ok {
				continue
			}
...
func (dcv *DateConsistencyValidator) validateCrossRecordDates(tree *types.GedcomTree, config *ValidationConfig) []*types.GedcomError {
	errors := make([]*types.GedcomError, 0)
	...
	for famXref, famRecord := range families {
		...
		var husbandBirth, wifeBirth *types.GedcomDate
		if husbandXref != "" {
			husbandRecord := tree.GetIndividual(husbandXref)
			if husbandRecord != nil {
				husband, ok := husbandRecord.(*types.IndividualRecord)
				if ok {
		...
		if wifeXref != "" {
			...
			if wifeRecord != nil {
				wife, ok := wifeRecord.(*types.IndividualRecord)
				if ok {
		...
		if len(children) > 1 {
			for i := 0; i < len(children)-1; i++ {
				...
				child1, ok1 := child1Record.(*types.IndividualRecord)
				child2, ok2 := child2Record.(*types.IndividualRecord)
				if !ok1 || !ok2 {
					continue
				}
...
func (dcv *DateConsistencyValidator) calculateAge(startDate, endDate *types.GedcomDate) int {
