...
func TestValidator_Integration_ValidTree(t *testing.T) {
	...
	// Create submitter
	submLine2 := types.NewGedcomLine(0, "SUBM", "", "@U1@")
	nameLine := types.NewGedcomLine(1, "NAME", "Test Submitter", "")
	submLine2.AddChild(nameLine)
	submRecord := types.NewSubmitterRecord(submLine2)
	...
	nameLine3 := types.NewGedcomLine(1, "NAME", "Jane /Doe/", "")
	sexLine2 := types.NewGedcomLine(1, "SEX", "F", "")
...
func TestValidator_Integration_InvalidTree(t *testing.T) {
	...
	famLine := types.NewGedcomLine(0, "FAM", "", "@F1@")
	husbLine := types.NewGedcomLine(1, "HUSB", "@I999@", "") // Non-existent
	famLine.AddChild(husbLine)
	famRecord := types.NewFamilyRecord(famLine)
	tree.AddRecord(famRecord)
