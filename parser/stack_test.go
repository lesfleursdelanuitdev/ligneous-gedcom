...
func TestLineStack_BasicOperations(t *testing.T) {
	...
	// Create test lines
	line1 := types.NewGedcomLine(0, "HEAD", "", "")
	line2 := types.NewGedcomLine(1, "GEDC", "", "")
	line3 := types.NewGedcomLine(2, "VERS", "5.5.5", "")
...
func TestLineStack_FindParent(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupStack: func() *LineStack {
				stack := NewLineStack()
				stack.Push(types.NewGedcomLine(0, "HEAD", "", ""))
				return stack
			},
			currentLevel: 1,
			wantParent:   true,
		...
		},
		{
			...
			setupStack: func() *LineStack {
				stack := NewLineStack()
				stack.Push(types.NewGedcomLine(0, "HEAD", "", ""))
				stack.Push(types.NewGedcomLine(1, "GEDC", "", ""))
				return stack
			},
			currentLevel: 2,
			wantParent:   true,
		...
		},
		{
			...
			setupStack: func() *LineStack {
				...
				stack.Push(types.NewGedcomLine(0, "HEAD", "", ""))
				stack.Push(types.NewGedcomLine(1, "GEDC", "", ""))
				stack.Push(types.NewGedcomLine(2, "VERS", "5.5.5", ""))
				return stack
			},
		...
		},
		{
			...
			setupStack: func() *LineStack {
				stack := NewLineStack()
				stack.Push(types.NewGedcomLine(0, "INDI", "", "@I1@"))
				stack.Push(types.NewGedcomLine(1, "NAME", "John /Doe/", ""))
				stack.Push(types.NewGedcomLine(2, "GIVN", "John", ""))
				return stack
			},
		...
		},
		{
			...
			setupStack: func() *LineStack {
				...
				stack.Push(types.NewGedcomLine(1, "NAME", "John /Doe/", ""))
				stack.Push(types.NewGedcomLine(2, "GIVN", "John", ""))
				stack.Push(types.NewGedcomLine(3, "NICK", "Johnny", ""))
				return stack
			},
	...
	tests := []struct {
		...
		},
		{
			...
			setupStack: func() *LineStack {
				stack := NewLineStack()
				stack.Push(types.NewGedcomLine(2, "VERS", "5.5.5", ""))
				stack.Push(types.NewGedcomLine(3, "FORM", "LINEAGE-LINKED", ""))
				return stack
			},
			targetLevel: 0,
			wantSize:    0,
...
func TestLineStack_RealWorldExample(t *testing.T) {
	...
	// 0 HEAD
	head := types.NewGedcomLine(0, "HEAD", "", "")
	stack.Push(head)
	if stack.Size() != 1 {
		t.Errorf("Expected size 1, got %d", stack.Size())
	}
	...
	// 1 GEDC
	gedc := types.NewGedcomLine(1, "GEDC", "", "")
	...
	// 2 VERS 5.5.5
	vers := types.NewGedcomLine(2, "VERS", "5.5.5", "")
	...
	char := types.NewGedcomLine(1, "CHAR", "UTF-8", "")
	parent, err = stack.FindParent(1)
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	...
	indi := types.NewGedcomLine(0, "INDI", "", "@I1@")
	...
	// 1 NAME John /Doe/
	name := types.NewGedcomLine(1, "NAME", "John /Doe/", "")
	parent, err = stack.FindParent(1)
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	...
	// 2 GIVN John
	givn := types.NewGedcomLine(2, "GIVN", "John", "")
	parent, err = stack.FindParent(2)
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	...
	surn := types.NewGedcomLine(2, "SURN", "Doe", "")
	parent, err = stack.FindParent(2)
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
