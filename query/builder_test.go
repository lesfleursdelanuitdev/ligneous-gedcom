...
func TestBuildGraph_FamilyEdges(t *testing.T) {
	...
	indi3Line.AddChild(types.NewGedcomLine(1, "NAME", "Child /Doe/", ""))
...
func TestBuildGraph_ReferenceEdges(t *testing.T) {
	...
	indiLine.AddChild(types.NewGedcomLine(1, "SOUR", "@S1@", ""))
	indi := types.NewIndividualRecord(indiLine)
	tree.AddRecord(indi)

	// Create note
	noteLine := types.NewGedcomLine(0, "NOTE", "", "@N1@")
	noteLine.AddChild(types.NewGedcomLine(1, "CONT", "This is a note", ""))
	note := types.NewNoteRecord(noteLine)
	tree.AddRecord(note)

	// Create source
	sourceLine := types.NewGedcomLine(0, "SOUR", "", "@S1@")
	sourceLine.AddChild(types.NewGedcomLine(1, "TITL", "Source Title", ""))
...
func TestBuildGraph_EventNodes(t *testing.T) {
	...
	birtLine.AddChild(types.NewGedcomLine(2, "PLAC", "New York", ""))
...
func TestBuildGraph_CompleteExample(t *testing.T) {
	...
	birt1Line.AddChild(types.NewGedcomLine(2, "DATE", "1 JAN 1850", ""))
	...
	marrLine.AddChild(types.NewGedcomLine(2, "DATE", "1 JAN 1875", ""))
