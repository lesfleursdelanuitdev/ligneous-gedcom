...
func TestHybridStorage_AllEdgeTypes(t *testing.T) {
	...
	indi1Line.AddChild(types.NewGedcomLine(1, "NOTE", "@N1@", ""))
	indi1Line.AddChild(types.NewGedcomLine(1, "SOUR", "@S1@", ""))
	indi1Line.AddChild(types.NewGedcomLine(1, "BIRT", "", ""))
	indi1Line.AddChild(types.NewGedcomLine(2, "DATE", "15 JAN 1800", ""))
	indi1 := types.NewIndividualRecord(indi1Line)
	tree.AddRecord(indi1)

	// Note
	note1Line := types.NewGedcomLine(0, "NOTE", "", "@N1@")
	note1Line.AddChild(types.NewGedcomLine(1, "CONT", "This is a note", ""))
	note1Line.AddChild(types.NewGedcomLine(1, "SOUR", "@S1@", ""))
	note1 := types.NewNoteRecord(note1Line)
	tree.AddRecord(note1)

	// Source
	source1Line := types.NewGedcomLine(0, "SOUR", "", "@S1@")
	source1Line.AddChild(types.NewGedcomLine(1, "TITL", "Source Title", ""))
	source1Line.AddChild(types.NewGedcomLine(1, "REPO", "@R1@", ""))
	source1 := types.NewSourceRecord(source1Line)
	tree.AddRecord(source1)

	// Repository
	repo1Line := types.NewGedcomLine(0, "REPO", "", "@R1@")
	repo1Line.AddChild(types.NewGedcomLine(1, "NAME", "Repository Name", ""))
	repo1 := types.NewRepositoryRecord(repo1Line)
	tree.AddRecord(repo1)

	// Family
	fam1Line := types.NewGedcomLine(0, "FAM", "", "@F1@")
	fam1Line.AddChild(types.NewGedcomLine(1, "HUSB", "@I1@", ""))
	fam1Line.AddChild(types.NewGedcomLine(1, "NOTE", "@N1@", ""))
	fam1Line.AddChild(types.NewGedcomLine(1, "SOUR", "@S1@", ""))
